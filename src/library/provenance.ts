...
export function resolveFromClause(library: LibraryLookup, clauseId: string, key: string): Resolution {
  ...
  const param = clause.params.find((p) => p.key === key);
  if (!param) return { usable: false, reason: `${clause.ref} does not state ${key}.` };
  ...
  return { usable: true, value: param.value, unit: param.unit, provenance: { kind: 'clause', clauseId }, warnings };
}
